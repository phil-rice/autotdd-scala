...
object IfThenParser {
  ...
  def parser1[P, R](becauses: Map[String, Because[(P) => Boolean]], inputs: Map[String, Any], constraints: Map[String, Constraint1[P, R]], thens: Map[String, CodeFn[(P) => R]]) = {
  ...
  def parser2[P1, P2, R](becauses: Map[String, Because[(P1, P2) => Boolean]], inputs: Map[String, Any], constraints: Map[String, Constraint2[P1, P2, R]], thens: Map[String, CodeFn[(P1, P2) => R]]) =
...
trait IfThenParser[R] extends EngineTypes[R] {
  ...
  case class ActualParser[R](val becauses: Map[String, Because[B]], val inputs: Map[String, Any], val constraints: Map[String, C], val thens: Map[String, CodeFn[RFn]], constraintMaker: (Because[B], List[Any], Code) => C) extends JavaTokenParsers with Function[String, RorN] {

    def resultOrifThenElse: Parser[RorN] = ifThenElse | result
    def ifThenElse: Parser[RorN] = "if" ~> because ~ inputList ~ constraintList ~ yes ~ "else" ~ no ^^ { case b ~ i ~ c ~ y ~ e ~ n => Right(Node(b, i, c, y, n)) }
