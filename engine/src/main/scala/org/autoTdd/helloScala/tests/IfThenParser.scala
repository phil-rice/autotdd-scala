...
class IfThenParser[B, R](val becauses: Map[String, B], val inputs: Map[String, R], val thens: Map[String, R]) extends JavaTokenParsers with Function[String, Node[B, R]] {
  type TE = Either[CodeFn[R], Node[B, R]]

  def ifThenElse: Parser[TE] = "if" ~> because ~ inputList ~ yes ~ "else" ~ no ^^ { case b ~ i ~ y ~ e ~ n => Right(Node(Because[B](b, b.toString), i, y, n)) }
  ...
  def result: Parser[TE] = ident ^^ (x => (Left(CodeFn[R](thens(x), thens(x).toString))))
